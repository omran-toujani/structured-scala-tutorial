...
object AbstractTypes extends App {
  ...
  // This means that the concrete implementations define the actual type. Here’s an example:
  ...
  //To illustrate this, we now look at a program which deals with a sequence buffer that refers to a list of integers
  ...
  //Here is a version of the code above which only uses type parameters:
  ...
  //Note that we have to use variance annotations here (+T <: Seq[U]) in order to hide the
  //concrete sequence implementation type of the object returned from method newIntSeqBuf.
