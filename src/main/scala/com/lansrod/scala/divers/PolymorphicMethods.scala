...
object PolymorphicMethods extends App {
  ...
  //The method listOfDuplicates takes a type parameter A and value parameters x and length.
  //Value x is of type A. If length < 1 we return an empty list.
  //Otherwise we prepend x to the the list of duplicates returned by the recursive call.
  //(Note that :: means prepend an element on the left to a list on the right.)

  //In first example call, we explicitly provide the type parameter by writing [Int].
  //Therefore the first argument must be an Int and the return type will be List[Int].
