...
object GenericClass extends App {
  ...
  /*This implementation of a Stack class takes any type A as a parameter. This means the underlying list, var elements: List[A] = Nil,
   * can only store elements of type A. The procedure def push only accepts objects of type A (note: elements = x :: elements reassigns
   * elements to a new list created by prepending x to the current elements).
   */
   ...
   * To conclude, Stack[A] is only a subtype of Stack[B] if and only if B = A. Since this can be quite restrictive,
   * Scala offers a type parameter annotation mechanism to control the subtyping behavior of generic types.
   */
}
