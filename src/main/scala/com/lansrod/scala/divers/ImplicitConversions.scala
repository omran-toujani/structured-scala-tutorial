 ...
 *An implicit conversion from type S to type T is defined by an implicit value which has function type S => T,
...
object ImplicitConversions extends App {
  ...
  //If an implicit method List[A] => Ordered[List[A]] is in scope, as well as an implicit method Int => Ordered[Int],
  //the following operation on the two lists of type List[Int] is legal: List(1, 2, 3) <= List(4, 5)
  ...
  //The implicitly imported object scala.Predef declares several aliases to frequently used types (e.g. scala.collection.immutable.Map
  //is aliased to Map) and methods (e.g. assert) but also several implicit conversions.
