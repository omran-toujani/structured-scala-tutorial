...
object PackagesAndImport {
  ...
  //However, Scala is agnostic to file layout. The directory structure of an sbt project for package users might look like this:
  ...
  //Notice how the users directory is within the scala directory and how there are multiple Scala files within the package.
  //Each Scala file in the package could have the same package declaration. The other way to declare packages is by using braces:
  ...
  //The package name should be all lower case and if the code is being developed within an organization which has a website,
  //it should be the following format convention: <top-level-domain>.<domain-name>.<project-name>.
  //For example, if Google had a project called SelfDrivingCar, the package name would look like this:
